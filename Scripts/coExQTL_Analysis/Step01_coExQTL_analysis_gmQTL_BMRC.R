...
#--------
print(treat[j])
gQTL_coExQTL=fread(paste0('coExQTL/gQTL_coExQTL/', treatment,'_coExQTL_gQTL.txt'), stringsAsFactors=F, header=T)
gQTL_coExQTL=as.data.frame(gQTL_coExQTL)
gQTL_coExQTL=gQTL_coExQTL[which(gQTL_coExQTL$pValDiff_adj<=1e-5),]
gQTL_coExQTL=gQTL_coExQTL[order(gQTL_coExQTL$pValDiff_adj,decreasing=F),]
gQTL_coExQTL=gQTL_coExQTL[!duplicated(gQTL_coExQTL$Gene2),]
dim(gQTL_coExQTL)
#--
...
i=1
for(i in 1:dim(gQTL_coExQTL)[1]){
  tryCatch({
    
    print(i)
    
    coExQTL_temp = gQTL_coExQTL[i,]
    
    Expression_subset = expression[which(row.names(expression) %in% GTF$id[which(GTF$symbol %in% coExQTL_temp$Gene2)] ),]
    dim(Expression_subset)
    
    TP_GENOTYPE_sub = TP_GENOTYPE[which(row.names(TP_GENOTYPE) == coExQTL_temp$var_id),]
    dim(TP_GENOTYPE_sub)
    
    Methylation = expression_methylation[-grep('ENSG', row.names(expression_methylation)),]
    dim(Methylation)
    ...
    ddcor_G2_G0=ddcorAll(inputMat=INPUT,design=design_matrix,compare=c("G2","G0"),adjust="fdr",nPerm=0,corrType="pearson",splitSet=GTF$id[which(GTF$symbol %in% coExQTL_temp$Gene2)], sigThresh=0.05,sortBy="pValDiff_adj",verbose=TRUE)
    ...
    ddcor_G2_G1=ddcorAll(inputMat=INPUT, design=design_matrix, compare=c("G2","G1"),adjust="fdr", nPerm=0, corrType="pearson", splitSet=GTF$id[which(GTF$symbol %in% coExQTL_temp$Gene2)], sigThresh=0.05, sortBy="pValDiff_adj", verbose=TRUE)

    colnames(ddcor_G2_G0) = c('Gene1','Gene2','GM_cor','GM_pVal','GR_cor','GR_pVal','zScoreDiff','pValDiff','pValDiff_adj','Classes')
    colnames(ddcor_G2_G1) = c('Gene1','Gene2','GM_cor','GM_pVal','GR_cor','GR_pVal','zScoreDiff','pValDiff','pValDiff_adj','Classes')
    
    ddcor_G2_G0 = cbind(ddcor_G2_G0, coExQTL_temp[,c('var_id', 'REF', 'ALT')])
    ddcor_G2_G1 = cbind(ddcor_G2_G1, coExQTL_temp[,c('var_id', 'REF', 'ALT')])
    
    ddcor_G2_G0$index = 'G2_G0'
    ddcor_G2_G1$index = 'G2_G1'
    
    ddcor_G2_G0$Gene2_name = coExQTL_temp$Gene2
    ddcor_G2_G1$Gene2_name = coExQTL_temp$Gene2
...
fwrite(RESULT, paste0('/exafs1/well/parkkinen/users/gbf362/Analysis_data/ciseQTL_Monocyte/coExQTL/gmQTL_coExQTL/',treatment,'_coExQTL_gmQTL.txt'),quote=F,row.names=F,sep='\t')
