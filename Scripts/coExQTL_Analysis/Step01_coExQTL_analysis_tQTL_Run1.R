...
dir.create('coExQTL/tQTL_coExQTL/', recursive = T)

#--- gene symbol to Ensemble transcript IDs
library(rtracklayer)
GTF <- rtracklayer::import('GENOMEANNOT/GRCh38.gtf')
...
for(i in 1:dim(cis_sub)[1]) {
  tryCatch({
    ...
    QTL_temp = cis_sub[i,] # cis[which(cis$var_id == 'rs2243999' & cis$phe_id == 'ENST00000468864'),]
    ...
    {
      for(g in 1:length(ddcor_G2_G1$Gene1))
      {
        ddcor_G2_G1$Gene1_name[g] = GTF_subset[which(GTF_subset$transcript_id %in% ddcor_G2_G1$Gene1[g]), 'gene_name']
        ddcor_G2_G1$Gene2_name[g] = GTF_subset[which(GTF_subset$transcript_id %in% ddcor_G2_G1$Gene2[g]), 'gene_name']
      }
    }
    
    if(dim(ddcor_G2_G0)[1]>0)
    {
      for(g in 1:length(ddcor_G2_G0$Gene1))
      {
        ddcor_G2_G0$Gene1_name[g] = GTF_subset[which(GTF_subset$transcript_id %in% ddcor_G2_G0$Gene1[g]), 'gene_name']
        ddcor_G2_G0$Gene2_name[g] = GTF_subset[which(GTF_subset$transcript_id %in% ddcor_G2_G0$Gene2[g]), 'gene_name']
      }
    }
    #---
    ...
    # {
    #--- save results
    write.table(RESULT, paste0('coExQTL/tQTL_coExQTL/', treatment,'_coExQTL_tQTL.txt'), quote = F, row.names = F, sep = '\t')
    # } 
